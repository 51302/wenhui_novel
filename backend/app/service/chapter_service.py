...
class ChapterService:
    ...
    async def _ensure_memory_chain(novel_unique_id: str, db: Session = None, current_chapter_num: int = 1) -> str:
        ...
        num_map = {}
        for c in published:
            n = _chapter_num(c.chapter_name)
            num_map.setdefault(n, []).append(c.chapter_name)
        for n, names in num_map.items():
            if len(names) > 1 and n < 99999:
                system_logger.warning(f"[三源校验] 检测到重复章节号 第{n}章: {names}，记忆体可能包含重复数据")
    ...
    async def generate_with_ai(db: Session, novel_unique_id: str, user_id: int,
        ...
        # ============================================================
        # 后处理：上限放宽到 2.5 倍（给AI更多写作空间，不强行截断）
        # ============================================================
        max_allowed = int(word_count * 2.5)
    ...
    async def regenerate_with_ai(db: Session, chapter_unique_id: str, user_id: int,
        ...
        async with httpx.AsyncClient(timeout=180) as client:
            try:
                ...
                # 重建记忆体：清除旧条目 → AI提取新内容 → 写入Redis
                try:
                    await ChapterService._rebuild_memory_for_chapter(
                        novel_unique_id, chapter_name, generated_text,
                        chapter_name, chapter_summary or ""
                    )
                    system_logger.info(f"[重新生成] {chapter_name} 记忆体已更新")
                except Exception as mem_err:
                    system_logger.error(f"[重新生成] {chapter_name} 记忆体更新失败: {mem_err}")
    ...
    @staticmethod
    async def continue_with_ai(db: Session, chapter_unique_id: str, word_count: int = 2000) -> dict:
